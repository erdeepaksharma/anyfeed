...
<div class="container-fluid">
	...
	<div class="buttons pull-right">
            <a href='index.php?route=feed/any_feed_pro/manageProfile&token=<?php echo $token ?>' class="button btn btn-primary" style="background-color: coral; border-color: coral;">Back to Profile</a>
...
function saveFeedsAjax() {
	...
	var url = 'index.php?route=feed/any_feed_pro/saveSettingsFieldsAjax&token=<?php echo $token ?>&id=<?php echo $id ?>';
	$.ajax({
		type: "POST",
		url: url,
		data: data,
		success: function(result) {
			if (result) {
				...
				setTimeout(function(){ $('.success').fadeOut() }, 10000);
                                // window.location.href='index.php?route=feed/any_feed_pro/manageProfile&token=<?php echo $token ?>';
			}
		},
...
$(document).ready(function() {
	...
	for(var key in existingFeeds) {
		createFeed(key, existingFeeds[key], 1);
	}
    ...
    $( ".portlet" ).addClass( "ui-widget ui-widget-content ui-helper-clearfix ui-corner-all" )
         .find( ".portlet-header" )
             .addClass( "ui-widget-header ui-corner-all a" )
	...
	function createFeed (profile, name, preset, callback) {
		...
		var json_url = 'index.php?route=feed/any_feed_pro/getProfile&feed='+ profile +'&preset=' + preset + '&token=<?php echo $token ?>&id=<?php echo $id ?>';
		...
		html = '<div class="field_content panela" style="width: 100%;">';
			addedDefaults = false;
